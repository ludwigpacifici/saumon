...
let check_value_list = Alcotest.(check (list A.value)) "check Interpreter.execute_k"

let check_option_value = Alcotest.(check (option A.value)) "check Interpreter.execute_k"
...
let make_program_exn (str : string) : Ast.Program.t =
  Scanner.scan_tokens str |> Caml.Result.get_ok |> Parser.parse |> Caml.Result.get_ok

(* Execute a program always with an empty environment. Useful for brief and comprehensives tests. *)
let execute_with_empty_env ~(k : Environment.t -> Value.t list -> unit) (p : Ast.Program.t) =
