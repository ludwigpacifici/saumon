...
let valid_declaration_with_no_initialization () =
  check_parse
    ...
    ( Ast.Variable_declaration {var; identifier = Ast.Identifier raw_identifier; assign = None; semicolon}
            ...
            Some
              ...
              ; expr =
                  Ast.Binary
                    ...
                    ; operator = {Token.kind = Token_kind.Plus; location = {Location.line = 1; column = 0}}
...
let only_var_keyword_is_error () = check_true (Parser.parse [var] |> Result.is_error)

let missing_initialization_or_semicolon_is_error () = check_true (Parser.parse [var; identifier] |> Result.is_error)

let not_equal_after_identifier_is_error () = check_true (Parser.parse [var; identifier; var] |> Result.is_error)
...
let not_expression_afer_equal_is_error () =
  check_true (Parser.parse [var; identifier; equal; identifier] |> Result.is_error)
...
let not_semicolon_after_initialization_is_error () =
  check_true (Parser.parse [var; identifier; equal; value; var] |> Result.is_error)

let all =
  [ Alcotest.test_case "Valid declaration with no initialization" `Quick valid_declaration_with_no_initialization
  ; Alcotest.test_case "Valid declaration with initialization" `Quick valid_declaration_with_initialization
  ; Alcotest.test_case "Valid declaration with expression" `Quick valid_declaration_with_expression
  ; Alcotest.test_case "No declaration" `Quick no_declaration
  ; Alcotest.test_case "Empty token list" `Quick empty_token_list
  ; Alcotest.test_case "Only var keyword is error" `Quick only_var_keyword_is_error
  ; Alcotest.test_case "Missing initialization or semicolon is error" `Quick
      missing_initialization_or_semicolon_is_error
  ; Alcotest.test_case "Not equal after identifier is error" `Quick not_equal_after_identifier_is_error
  ; Alcotest.test_case "Missing initialization after equal is error" `Quick missing_initialization_after_equal_is_error
  ; Alcotest.test_case "Not expression afer equal is error" `Quick not_expression_afer_equal_is_error
  ; Alcotest.test_case "Missing semicolon after initialization is error" `Quick
      missing_semicolon_after_initialization_is_error
  ; Alcotest.test_case "Not semicolon after initialization is error" `Quick not_semicolon_after_initialization_is_error
  ]
