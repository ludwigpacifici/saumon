...
let semicolon_missing_after_expression_statement () =
  check_true
    (Parser.parse [Token.of_token_kind ~kind:Token_kind.Nil] |> Result.is_error)
...
let all =
  ...
  ; Alcotest.test_case "Semicolon missing after print statement" `Quick
      semicolon_missing_after_print_statement ]
