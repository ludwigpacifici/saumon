...
let evaluate_binary_number (l : float) (t : Token.t) (r : float) : (Value.t, error) Result.t =
  let open Poly in
  match t.kind with
  ...
  | t_kind ->
      Error
        { location = t.location
        ; message = "Unknown binary operator (" ^ Token_kind.to_string t_kind ^ ") to be used with two floats" }

let evaluate_binary_string (l : string) (t : Token.t) (r : string) : (Value.t, error) Result.t =
  match t.kind with
  | Token_kind.Plus -> Ok (Value.String (l ^ r))
  | t_kind ->
      Error
        { location = t.location
        ; message = "Unknown binary operator (" ^ Token_kind.to_string t_kind ^ ") to be used with two strings" }

let rec evaluate (env : Environment.t) (expr : Ast.expression) : (Environment.t * Value.t, error) Result.t =
  match expr with
  ...
  | Ast.Binary {left_expr; operator; right_expr} -> evaluate_binary env left_expr operator right_expr
      ...
      if Environment.contains ~env ~id then
        ...
        | None ->
            Error
              ...
              ; message = "Unable to do assignement for " ^ id ^ " even though it is already defined." }
      else Error {location = equal.location; message = "Undefined variable '" ^ id ^ "'"}
  | Ast.Assignment {equal; _} ->
      Error {location = equal.location; message = "Expected to have an identifer on the left of '='"}

and evaluate_literal (env : Environment.t) (l : Ast.literal) : (Environment.t * Value.t, error) Result.t =
  match l with
  | Ast.Identifier id -> (
    (* Value.of_ast_literal does not resolve literal identifier *)
    match Environment.get ~env ~id with
    | Some v -> Ok (env, v)
    | None -> Error {location = Location.make ~line:0 ~column:0; message = "Undefined variable: " ^ id} )
  | l -> Ok (env, Value.of_ast_literal l)

and evaluate_unary (env : Environment.t) (t : Token.t) (e : Ast.expression) : (Environment.t * Value.t, error) Result.t
    =
  match (t.kind, evaluate env e) with
  | Token_kind.Minus, Ok (env, Value.Number e) -> Ok (env, Value.Number (-.e))
  | Token_kind.Minus, Ok _ -> Error {location = t.location; message = "Expected a float after a unary minus"}
  | Token_kind.Bang, Ok (env, v) ->
      let b = not (Value.is_truthy v) in
      Ok (env, Value.Bool b)
  | t_kind, _ -> Error {location = t.location; message = "Unknown unary (" ^ Token_kind.to_string t_kind ^ ") operator"}
...
and evaluate_binary (env : Environment.t) (l : Ast.expression) (t : Token.t) (r : Ast.expression) :
    (Environment.t * Value.t, error) Result.t =
  let greedy_evaluate (l : Ast.expression) (t : Token.t) (r : Ast.expression) =
    match (evaluate env l, t.kind, evaluate env r) with
    | Ok (_, l), Token_kind.Equal_equal, Ok (_, r) -> Ok (env, Value.Bool (Value.equal l r))
    | Ok (_, l), Token_kind.Bang_equal, Ok (_, r) -> Ok (env, Value.Bool (Value.equal l r |> not))
    ...
    | Error _, _, Ok _ -> Error {location = t.location; message = "Not able to evaluate the left expression"}
    | Ok _, _, Error _ -> Error {location = t.location; message = "Not able to evaluate the right expression"}
    | Error _, _, Error _ ->
        Error {location = t.location; message = "Not able to evaluate both left and right expression"}
  in
  match t.kind with
...
let rec execute_statement (env : Environment.t) (s : Ast.statement) :
  ...
  let rec while_loop condition body (env, values) =
    ...
    if Value.is_truthy condition_value then
      ...
      | Some values, Some new_values -> while_loop condition body (env, Some (new_values @ values))
    else Ok (env, values)
  in
  match s with
  ...
  | Ast.Expression_statement {expr} ->
      ...
      (* Evaluate the expression in case of errors, but discard on purpose the computed value because there is no print
         statement for it. Forward the environment because it can be updated by an assignment. *)
      Ok (env, None)
  | Ast.If_statement {condition; if_body; else_body} -> (
      ...
      else match else_body with None -> Ok (env, None) | Some else_body -> execute_statement env else_body )
  | Ast.Print_statement {expr} ->
      let* env, v = evaluate env expr in
      Ok (env, Some [v])
  | Ast.While_statement {condition; body} -> while_loop condition body (env, None)

(* Run everything in the block and ensure the environment is pushed/updated/popped correctly. *)
and execute_block env ds =
  ...
  | None ->
      Error
        { location = (* FIXME: review error reporting with to provide line/column *)
                     Location.start ()
        ; message = "Closing a block will remove the whole environment" }

and execute_variable_declaration (env : Environment.t) (v : Ast.variable_declaration) : (Environment.t, error) Result.t
    =
  match v with
  ...
  | {var; _} -> Error {location = var.location; message = "Cannot execute a variable declaration."}
...
let extract_values ((env : Environment.t), (vs : Value.t list)) (d : Ast.declaration) =
...
let execute_k ~(k : Environment.t -> Value.t list -> unit) (env : Environment.t) (program : Ast.Program.t) :
    (unit, error) Result.t =
  let+ env, vs = List.fold_result ~init:(env, []) ~f:extract_values (Ast.Program.get program) in
  k env (List.rev vs)

let execute (env : Environment.t) (program : Ast.Program.t) : (unit, error) Result.t =
