...
let main_exn exe_name args in_channel input_mode : int =
  match input_mode with Repl -> Repl.run exe_name args in_channel | Script -> Script.run args in_channel
...
let command exe_name =
  Command.basic ~summary:(exe_name ^ ": An interpreter for Lox")
    Command.Let_syntax.(
      let%map_open filename = anon (maybe_with_default "-" ("filename" %: string))
      and print_scanner = flag "-s" no_arg ~doc:" Print scanner outputs on stdout"
      and print_parser = flag "-p" no_arg ~doc:" Print parser outputs on stdout" in
      fun () ->
